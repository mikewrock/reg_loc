	...
	while( i < (planes_loc.size()-1) && planes_loc.size() >= 2 ){
		...
		while(  j < (planes_loc.size())){
				...
				if(fabs(fabs(value)-DOT) < best_value){
					...
					if(holder(2) > 0){
					if(value > 0) parallel1 = normalized_cross_product(purpendicular,planes_loc.at(i));

					else	parallel1 = normalized_cross_product(planes_loc.at(i),purpendicular);
						parallel2 = normalized_cross_product(purpendicular,planes_loc.at(j));
					}
					else{
					if(value > 0) {parallel2 = normalized_cross_product(purpendicular,planes_loc.at(i));
...
sensor_msgs::PointCloud2 output, output1, output3;
